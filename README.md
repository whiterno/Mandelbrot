# Анализ различных стратегий оптимизации вычислений на основе отрисовки множества Манельброта
...
По итогу будет составлена таблица, в которой можно будет увидеть среднее значение FPS для каждрй версии, погрешность измерений и увеличение производительности по сравнению с базовой версией без оптимизаций.
...
В левом верхнем углу будет показываться FPS. С помощью следующий клавиш можно перемещаться и изменять масштаб:
...
Мы не можем построить бесконечную последовательность, поэтому будем проверять, если точка не вышла за границу $R$ на `ITERATIONS = 1024`, то она принадлежит множеству и имеет черный цвет, иначе множеству она не принадлежит. В изначальном варианте такие точки имеют белый цвет, но для более интересной картинки можно задать функцию от итерации, на которой точка вышла за границу $R$.

Если же разделить комплексную точку на действительную и мнимую часть, то получим следующее:
$$c = x_{0} + iy_{0}$$
$$x_{n+1} = x_{n}^{2} - y_{n}^{2} + x_{0}$$
$$y_{n+1} = 2x_{n}y_{n} + y_{0}$$

В базовой версии без оптимизации считаем через комплексную точку (с помощью созданой структуры `Complex_t`).

В версии с SIMD и разверткой циклов используем разделение на действительную и мнимую часть, так как это позволяет нам применять данные оптимизации.
...
Все довольно грустно, каждая точка отдельная просчитывается в функции, записывается в массив, даже флаг `-O3` с таким не справляется при данном разрешении и количестве итераций. Процессор на загружен, мы подаем ему задачи последовательно. Можно это исправить. Есть способ распараллелить данные - использовать векторные инструкции, принцип SIMD. Мы можем просчитывать сразу несколько точек одновременно.

Тестирование проходит на машине с процессором Apple M3 Pro, который имеет архитектуру ARM, поэтому будем пользоваться набором команд ARM NEON. Максимальная битность регистра - 128, поэтому одновремеено можно обрабатывать только 4 значение float (по 32 бита каждый).
...
Мы смогли увеличить производительность в 5,5 раз! Теперь воспользуемся другим методом оптимизаций - разверткой циклов. Будем обрабатывать несколько точек за 1 итерацию цикла. Такой метод позволяет уменьшать количество проверок цикла, соответственно связанные с циклом риски (ложное предсказание при jmp), а так же заполнять конвейер процессора, чтобы он орбабатывал 2, 3 и т.д. точки вместо того, чтобы проверять условия перехода или впустую работать из-за ложного предсказания.

В таком случае нужно позабоиться о написании кода, в котором не будет зависимостей по данным, чтобы выжать максимум из данного метода. Присутствуют и минусы: при развертывании больших циклов растет объем кода, что в свою очередь негативно сказывается на обращении к памяти и попаданию по кешу.
